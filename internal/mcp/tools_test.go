		...
		},
		{
			...
			setupClient: func() *mockGitLabClient {
				return &mockGitLabClient{
					...
					updateMRAssigneesFunc: func(projectID, iid int, assigneeIDs []int) (*gitlab.MergeRequest, error) {
						if len(assigneeIDs) != 1 || assigneeIDs[0] != 99 {
							t.Errorf("assigneeIDs = %v, want [99]", assigneeIDs)
						}
						return &gitlab.MergeRequest{ID: 1, IID: 10, ProjectID: 42}, nil
					},
				}
...
func TestMRAssigneeHandler(t *testing.T) {
	tests := []struct {
		name    string
		input   MRAssigneeInput
		setup   func() *mockGitLabClient
		wantErr bool
	}{
		{
			name:  "list assignees",
			input: MRAssigneeInput{ProjectID: 1, MRIID: 10},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					getMRFunc: func(_, _ int) (*gitlab.MergeRequest, error) {
						return &gitlab.MergeRequest{
							Assignees: []gitlab.User{{ID: 1, Username: "dev"}},
						}, nil
					},
				}
			},
		...
		{
			name:  "add assignee",
			input: MRAssigneeInput{ProjectID: 1, MRIID: 10, Add: []int{42}},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					getMRFunc: func(_, _ int) (*gitlab.MergeRequest, error) {
						return &gitlab.MergeRequest{
					...
					updateMRAssigneesFunc: func(_, _ int, ids []int) (*gitlab.MergeRequest, error) {
						return &gitlab.MergeRequest{
							Assignees: []gitlab.User{{ID: 1}, {ID: 42}},
						}, nil
					},
				}
			},
		},
		{
			...
			input: MRAssigneeInput{ProjectID: 1, MRIID: 10, Remove: []int{1}},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					getMRFunc: func(_, _ int) (*gitlab.MergeRequest, error) {
						return &gitlab.MergeRequest{
							Assignees: []gitlab.User{{ID: 1, Username: "dev"}, {ID: 42, Username: "dev2"}},
						}, nil
					},
					updateMRAssigneesFunc: func(_, _ int, ids []int) (*gitlab.MergeRequest, error) {
						if len(ids) != 1 || ids[0] != 42 {
							t.Errorf("assigneeIDs = %v, want [42]", ids)
						}
						return &gitlab.MergeRequest{
							Assignees: []gitlab.User{{ID: 42, Username: "dev2"}},
						}, nil
					},
				}
			},
		},
		{
			...
			input: MRAssigneeInput{ProjectID: 1, MRIID: 10, Add: []int{99}, Remove: []int{1}},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					getMRFunc: func(_, _ int) (*gitlab.MergeRequest, error) {
						return &gitlab.MergeRequest{
					...
					updateMRAssigneesFunc: func(_, _ int, ids []int) (*gitlab.MergeRequest, error) {
						if len(ids) != 1 || ids[0] != 99 {
							t.Errorf("assigneeIDs = %v, want [99]", ids)
						}
						return &gitlab.MergeRequest{
							Assignees: []gitlab.User{{ID: 99, Username: "new"}},
						}, nil
					},
				}
			},
		},
		{
			...
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					...
					updateMRAssigneesFunc: func(_, _ int, _ []int) (*gitlab.MergeRequest, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := testServer(tt.setup())
			_, _, err := s.MRAssigneeHandler(context.Background(), nil, tt.input)
...
func TestMRUpdateHandler(t *testing.T) {
	tests := []struct {
		name        string
		input       MRUpdateInput
		setup       func() *mockGitLabClient
	...
	}{
		{
			name: "update title",
			input: MRUpdateInput{
				ProjectID: 1, MRIID: 10,
				Title: strPtr("New Title"),
			},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					updateMRFunc: func(pid, iid int, opts gitlab.UpdateMROptions) (*gitlab.MergeRequest, error) {
						if opts.Title == nil || *opts.Title != "New Title" {
							t.Errorf("title = %v, want 'New Title'", opts.Title)
						}
						return &gitlab.MergeRequest{
							ID: 100, IID: 10, ProjectID: 1, Title: *opts.Title,
							Author: gitlab.User{Username: "dev"},
						}, nil
					},
				}
		...
		},
		{
			...
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					updateMRFunc: func(pid, iid int, opts gitlab.UpdateMROptions) (*gitlab.MergeRequest, error) {
						if opts.Squash == nil || !*opts.Squash {
		...
		},
		{
			...
			input: MRUpdateInput{ProjectID: 1, MRIID: 10},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					updateMRFunc: func(_, _ int, _ gitlab.UpdateMROptions) (*gitlab.MergeRequest, error) {
		...
		},
		{
			...
			input: MRUpdateInput{ProjectID: 1, MRIID: 10, StateEvent: strPtr("merged")},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{}
			},
			wantErr:     true,
			errContains: "state_event must be",
		},
		{
			name:  "API error",
			input: MRUpdateInput{ProjectID: 1, MRIID: 10, Title: strPtr("Fail")},
			setup: func() *mockGitLabClient {
				return &mockGitLabClient{
					updateMRFunc: func(_, _ int, _ gitlab.UpdateMROptions) (*gitlab.MergeRequest, error) {
						return nil, errors.New("forbidden")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := testServer(tt.setup())
			_, output, err := s.MRUpdateHandler(context.Background(), nil, tt.input)
