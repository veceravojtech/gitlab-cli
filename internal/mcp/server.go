...
type GitLabClient interface {
	...
	UpdateMRAssignees(projectID, iid int, assigneeIDs []int) (*gitlab.MergeRequest, error)
	UpdateMR(projectID, iid int, opts gitlab.UpdateMROptions) (*gitlab.MergeRequest, error)
...
func (s *Server) RegisterTools(sdkServer *sdkmcp.Server) {
	...
	sdkmcp.AddTool(sdkServer, &sdkmcp.Tool{
		Name:        "mr-assignee",
		Description: "Add or remove assignees on a merge request by user ID",
		Annotations: &sdkmcp.ToolAnnotations{
			IdempotentHint:  true,
			DestructiveHint: &falseVal,
		},
	...
	sdkmcp.AddTool(sdkServer, &sdkmcp.Tool{
		Name:        "mr-update",
		Description: "Update merge request properties: title, description, target branch, state, draft, squash, labels, assignees, reviewers, milestone, remove source branch, discussion lock, allow collaboration",
		Annotations: &sdkmcp.ToolAnnotations{
			IdempotentHint:  true,
			DestructiveHint: &falseVal,
		},
	...
	sdkmcp.AddTool(sdkServer, &sdkmcp.Tool{
		Name:        "mr-create",
		Description: "Create a new merge request with optional labels, reviewers, and assignees",
	}, s.MRCreateHandler)
